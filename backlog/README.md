...
> Blazor client stays published at `legado.entrelares.app` as the documented way back
> until its shutdown, which is ops with no date. Record in
> [`archive/phase-7.md`](archive/phase-7.md); runbook, rollback and the day the rollback
> dies in [`docs/flutter-cutover.md`](../docs/flutter-cutover.md).
...
| **F-57** | **Social login — Google now, Apple with T-40** | Added 12/08/2026 from closed-alpha feedback; owner: "o quanto antes" (Ordem 2 — it was slotted right after F-54 to catch the GoTrue config while warm; F-54 and F-58 are delivered, so it now sits behind F-53). Kills the confirmation-e-mail failure mode for most sign-ups. Consent gate (S-13) must move into the first OAuth login; **Apple half is a hard prerequisite of T-40** (App Store rule) and waits for it. |
| **L-16** *(landing)* | English version of the landing page | Pair of U-13, which is now **delivered** — the recruited tester reads the site first, so the app speaking English while the landing does not is the remaining half of the barrier. Rationale in the landing `ROADMAP.md`. |
| **T-47** | iOS publishability spike — real App Review verdict | Minimal Capacitor shell (bundled assets + APNs), submitted with manual release: Apple's verdict lands with zero users affected and **years before the iOS moment**. US$99/yr owner ops. Verdict is a prerequisite of T-40 and scopes it. |
| **T-40** | iOS App Store wrapper (Capacitor) | Promoted from last (owner, Aug 2026). Genuinely costlier than Android: native Capacitor shell, APNs push (couples with **F-09**, group 5), App Review's "thin wrapper" scrutiny, **US$99/yr**. Keeps T-38 as its prerequisite — and now **T-47's verdict** as scope input. The web-first billing (T-39) is what lets the iOS build stay a "manage your plan on the website" client and avoid Apple's IAP cut; re-verify the external-link rules at build time. |
...
| Item | What |
|---|---|
| **U-12** | Dark mode (CSS custom properties across all pages). |
...
| **F-09** | Web Push notifications (works when the app is closed; native-push companion for T-40). **Grew a scope half in Aug 2026**: the contextual opt-in flow — on iOS Web Push exists only for an installed PWA, so the flow branches on install state (`js/pwa.js` already detects it). |
| **T-18** | Offline-first data strategy (IndexedDB + background sync). **Split recommended (Aug 2026)**: the *read* half + a "Modo offline" indicator is the whole user-facing value; the write queue is a separate decision, because a queued write can be legitimately refused when it flushes (T-35 token, frozen days, F-39 horizon). |
| **U-21** | Profile page refinement — read-only groups + pencil-to-edit bottom sheets. |
| **T-50** | Move the Playwright `e2e-traces` off the GitHub artifact quota, to the **R2 bucket the weekly backup already uses (T-19)**. Created 07/08/2026 from an incident: a red `dev` E2E run had to be diagnosed by reading source, because the upload failed with *"Artifact storage quota has been hit"*. The 500 MB cap is **account-wide** and `e2e-traces` is the only artifact either repo produces, so it alone decides whether the quota holds — and the failure mode is the worst one available: you lose the diagnostic exactly when a test is failing. The Aug 2026 mitigations (`Screenshots` off, 1-day retention) bought headroom but left the ceiling. **One line worth doing regardless of scheduling**: `continue-on-error: true` on the upload step, so a diagnostic step can never change a verdict. |
...
| **T-57** | **Re-shoot the product screenshots** — the eight frames on the Play listing and the landing are from 23/07/2026, of the **Blazor** client under the pre-U-27 visual system, so the only picture a stranger sees is of a product that no longer exists. **Absorbs the landing's L-21** (24/08/2026): that item assumed the PT-BR captures were current and only the English set was missing — the cutover and U-27/U-28 killed that premise, and one capture session produces both. Shoot a PRODUCTION-configured build: the dev flavour prefixes `[Dev] ` into every stored notification title. |
