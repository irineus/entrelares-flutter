...
| **T-58** | **The web flow gate must prove it ran** — `web-e2e` blocks the web publish and was passing GREEN with its `setUpAll` throwing: on web `flutter drive` prints `All tests passed.` and exits 0 when zero tests ran, so `set -euo pipefail` cannot see it. It was born (#63, 24/08) five days after the bug that broke its `setUpAll` (19/08), so it never executed a test — the green that justified letting it block production was vacuous. The guard must require POSITIVE proof of execution, and be designed against the first genuinely green run, which has never existed. Carries the correction of the false "144 s / 5 testes" line in `docs/arquivamento-app.md`. Making it a real gate is a change of regime — owner's call. |
