...
| **T-57** | **Bring the whole product listing to the current app** — grew from "re-shoot the screenshots" to the entire public surface on 26/08/2026, after U-29 replaced the mark and a sweep found the frames were the least of it. Four parts, in **both languages**, published **together** (U-29 held the store icon precisely until this item releases it): the **mark** in three places — `store/store_icon.png`, the Console upload, and the landing's five brand files; the **feature graphic**; the **eight frames**, PRODUCTION-configured (the dev flavour prefixes `[Dev] ` into stored notification titles); and the **copy**, which is where it stops being cosmetic — the listing promised *"funciona também offline"* while **T-18 is still `pending`**, a claim the cutover falsified and nobody re-read. **Absorbs the landing's L-21** (24/08/2026). Also carries a finding to confirm: the Console's screenshots appear to be a pre-masking upload, showing a **DEV** pill the repo's files do not have. Nothing here ships by merge — the store half is manual Console uploads. **PR 1 (28/08/2026) closed the half that needs no running app**: the copy re-verified in both languages, `store_icon.png` back under `brand-icons.py`, and both feature graphics re-rendered on the new mark with a generator each. What is left is the eight frames, the landing's brand files, and every Console upload. |
